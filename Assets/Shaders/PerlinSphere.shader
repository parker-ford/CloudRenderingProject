        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                if(sphereIntersect.intersects == true){
                    ...
                    float steps = 10.;
                    float distPerStep = dist / steps;
                    float densityResult = 0.0f;
                    float density = 0.5f;
                    bool samplePerlin = true;
                    // for(int j = 0; j < steps; j++){
                    //     float currDist = ((float)j / (float)steps) * dist;
                    //     float3 pos = rayOrigin + rayDir * (sphereIntersect.intersectPoints.x + currDist);
                    ...
                    for(float j = 0; j < steps; j++){
                        // float currDist = lerp(0, dist,  j /  steps);
                        float3 pos = rayOrigin + rayDir * (sphereIntersect.intersectPoints.x + (distPerStep * j));
                        float3 samplePos = remap_f3(pos, -0.5, 0.5, 0.0, 1.0);
                        float4 noise = tex3D( _Perlin3DTexture, samplePos);
                        float noiseSample = (noise.x + 1) / 2.0f;
                        if(noiseSample > _NoiseThreshold){
                            densityResult = noiseSample;
                            break;
                        } 
                    }

                    return lerp( tex2D(_MainTex, i.uv), float4(1.0, 0.0, 0.0, 1.0), pow(densityResult,5));
                    // return lerp(float4(1.0, 0.0, 0.0, 1.0), tex2D(_MainTex, i.uv),  densityResult);
                } 
